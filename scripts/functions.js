$(document).ready(function() {
	...
	$(window).load(function() {
		...
		$(".pagination div").click(function() {
			if ($(this).index() != curnewsitemset.index()) {
				...
				curnewsitemset.animate({ opacity: "hide" }, { duration: 250,
					complete: function() {
	...
	if (numberofitems > 1) {
		...
		$(".next, .workeg, .nextvid div").click(function() {
			if (crslovrrd == false)
			{
				...
				next2workeg.css({ left: "309%" }).show().animate({ left: "206%" }, { duration: 900, easing: 'easeOutExpo',
					complete: function() {
						goneworkeg.hide();
						crslovrrd = false;
					}
		...
		$(".prev").click(function() {
			if (crslovrrd == false)
			{
				...
				prev2workeg.css({ left: "-309%" }).show().animate({ left: "-206%" }, { duration: 900, easing: 'easeOutExpo',
					complete: function() {
						goneworkeg.hide();
						crslovrrd = false;
					}
	...
	$(".subpage .loadscreen").css({ top: midpoint }).delay(600).animate({ height: 60, top: midpoint - 30 }, { duration: 400, easing: 'easeInExpo',
	...
	$(window).load(function() {
		...
		$(".subpage .loadscreenfg").delay(1000).animate({ opacity: "show" }, { duration: 600,
				complete: function() {
					...
					if (newcntheight > tempheight) {
						$(".subcontent").animate({ height: newcntheight }, { duration: 400, easing: 'easeInExpo',
		...
		$(window).load(function() {
			...
			$(".homeblock").hover(function() {
				if (hbdisable == false) {
					...
					$(this).find(".hbinfo").delay(300).animate({ top: "0" }, { duration: 150,
			...
			$(".homeblock").click(function() {
				if ($(this).attr("id") == hbcurrent && hbdisable == false) {
					...
					setTimeout(function() { window.location = hblink; }, 600 );
				}
			});
...
function loadanimation() {
	...
	$(".loadanim div:nth-child(8)").delay(700).animate({ opacity: "hide" }, { duration: 200,
